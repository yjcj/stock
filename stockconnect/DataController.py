...
def thsGetBlock():
    ...
    for ele in listt:
        ...
        # lists = tenjqka.fetch_stocks(ele[0])
        # for ele in lists:
        #     print(ele)
    #
def getstockbyid(id):
        lists = tenjqka_stub.fetch_stocks(id)
...
def getklinebystockid(id):
    listk = tenjqka_stub.fetch_ind_kline(id)
    for ele in listk:
        print(ele)
        sk = Thsstockkline.create(stockid=id, date=ele[0], begin=ele[1], max=ele[2], min=ele[3], end=ele[4],
